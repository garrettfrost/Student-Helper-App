...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.font = [UIFont fontWithName:@"DK Crayon Crumble" size:25.0];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        Subjects *subject = [self.subjects objectAtIndex:indexPath.row];
        NSArray *subjectToDelete = [Subjects MR_findByAttribute:@"subjectCode" withValue:subject.subjectCode];
        subject = [subjectToDelete objectAtIndex:0];
        
        NSArray *assessmentsToDelete = [Assessments MR_findByAttribute:@"subjectCode" withValue:subject.subjectCode];
        NSArray *classesToDelete = [Classes MR_findByAttribute:@"subjectCode" withValue:subject.subjectCode];
                
        if([assessmentsToDelete count] > 0)
        {
            for(int i = 0; i < assessmentsToDelete.count; i++)
                [[assessmentsToDelete objectAtIndex:i] MR_deleteEntity];
        }
        
        if([classesToDelete count] > 0)
        {
            for(int i = 0; i < classesToDelete.count; i++)
                [[classesToDelete objectAtIndex:i] MR_deleteEntity];
        }
        
        [[subjectToDelete objectAtIndex:0] MR_deleteEntity];
