...
- (void)viewDidLoad
{
    ...
    if (thisSession.count == 0)
    {
        ...
        needSession = [[UIAlertView alloc]
                       initWithTitle:@"Welcome To Student Helper!"
                       message:@"We just need you to select your current session"
                       delegate:self
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        static NSString *CellIdentifier = @"Timetable";
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
        ...
        if([thisSession count])
        {
            ...
            cell.textLabel.font = [UIFont fontWithName:@"DK Crayon Crumble" size:30.0];
            cell.detailTextLabel.font = [UIFont fontWithName:@"DK Crayon Crumble" size:19.0];
        }
        else
        {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
 ...
 - (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
 {
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        if(indexPath.section == 1)
        {
            Classes *classToDelete = [self.classArray objectAtIndex:indexPath.row];
            NSArray *deleteArray = [Classes MR_findByAttribute:@"classID" withValue:classToDelete.classID];
            classToDelete = [deleteArray objectAtIndex:0];
            [classToDelete MR_deleteEntity];
